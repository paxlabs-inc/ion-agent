...
metadata:
  ion:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, debugging-ion-tui-commands]
---
...
- A long-running process (ion gateway, tui_gateway) misbehaves and you can't restart it
...
The ion test runner and pytest both support this:
...
For long-lived processes: Ion gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/ion-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Ion you have two practical options:
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/ion-agent" }
  ]
}
```
...
## Debugging Ion-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `ion` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `ion --tui`)
...
debugpy.wait_for_client()
```
Start `ion --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Ion subagents, debug one process at a time.
