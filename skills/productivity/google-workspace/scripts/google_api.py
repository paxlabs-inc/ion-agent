#!/usr/bin/env python3
"""Google Workspace API CLI for Ion Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Ion-facing JSON contract and falls back to the Python client
...
from _ion_home import get_ion_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ION_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Ion Agent")
