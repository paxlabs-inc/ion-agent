#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Ion Agent.
...
# Ensure sibling modules (_ion_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _ion_home import display_ion_home, get_ion_home

ION_HOME = get_ion_home()
TOKEN_PATH = ION_HOME / "google_token.json"
CLIENT_SECRET_PATH = ION_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ION_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Ion profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Ion Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Ion Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'ion-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Ion home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_ion_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Ion")
