...
Interact with Notion through two approaches. A single integration token works for both — choose based on what's available.

◆ **`ntn` CLI** — Notion's official CLI. More concise syntax, one-liner file uploads, required for Workers. macOS + Linux only as of May 2026 (Windows support "coming soon"). **Default when installed.**
...
Add those exports to your shell profile (or to `${ION_HOME:-~/.ion}/.env`) so every session inherits them.
...
curl -s -X PATCH "https://api.notion.com/v1/blocks/{page_id}/children" \
  ...
  -d '{
    "children": [
      {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "Hello from Ion!"}}]}}
    ]
  }'
```
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via Ion' MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
