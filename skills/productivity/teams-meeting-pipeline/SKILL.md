...
description: "Operate the Teams meeting summary pipeline via Ion CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Activate this skill whenever the user brings up Microsoft Teams meeting summaries, transcripts, recordings, action items, Graph subscriptions, or any operational question about the Teams meeting pipeline. Works in any language — the triggers below are examples, not an exhaustive list.

Everything operator-facing is a `ion teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${ION_HOME:-~/.ion}/.env`:
...
```bash
ion teams-pipeline validate              # config snapshot — run first after any change
ion teams-pipeline token-health          # Graph token status
ion teams-pipeline token-health --force-refresh   # force a fresh token acquisition
ion teams-pipeline list                  # recent meeting jobs
ion teams-pipeline list --status failed  # only failed jobs
ion teams-pipeline show <job-id>         # full detail of one job
ion teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
ion teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
ion teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
ion teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
ion teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

ion teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
ion teams-pipeline delete-subscription <sub-id>
ion teams-pipeline maintain-subscriptions            # renew near-expiry ones
ion teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `ion teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `ion cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
