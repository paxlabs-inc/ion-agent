...
Reach for vLLM when deploying production LLM APIs, squeezing better inference latency/throughput, or serving models under tight GPU memory constraints. It provides OpenAI-compatible endpoints, quantization support (GPTQ/AWQ/FP8), and tensor parallelism.

## Quick start

vLLM delivers up to 24x higher throughput than vanilla transformers thanks to PagedAttention (block-based KV cache) and continuous batching (interleaving prefill/decode requests).
