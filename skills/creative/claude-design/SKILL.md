...
Apply this skill when the user requests design work that would typically belong in Claude Design, but the agent operates in a CLI/API environment rather than the hosted Claude Design web UI.

The objective is to retain Claude Design's useful design behavior and aesthetic judgment while stripping out hosted-tool plumbing that doesn't exist in standard agent environments.
...
Ion has three design-related skills under `skills/creative/`. They do different jobs — load the right one (or combine them):
