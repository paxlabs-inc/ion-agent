...
A collection of tools for various ASCII art requirements. All tools run as local CLI programs or use free REST APIs — no API keys needed.

## Tool 1: Text Banners (pyfiglet — local)

Transform text into large ASCII art banners. 571 built-in fonts available.
...
A free REST API that transforms text into ASCII art. 250+ FIGlet fonts available. Returns plain text directly — no parsing required. Use this when pyfiglet is absent or as a quick alternative.
...
A classic tool that wraps text inside a speech bubble with an ASCII character.
...
Draw ornamental ASCII art borders/frames around any text. 70+ built-in designs.
...
```bash
python3 -m pyfiglet "ION" -f slant | boxes -d stone
# Or without pyfiglet installed:
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=ION&font=Slant" | boxes -d stone
```

## Tool 5: TOIlet (Colored Text Art)

Similar to pyfiglet but with ANSI color effects and visual filters. Excellent for terminal eye candy.
...
A large collection of classic ASCII art organized by subject. Art is inside HTML `<pre>` tags. Fetch the page with curl, then extract art with a small Python snippet.
...
When the tools above don't have what's needed, generate ASCII art directly using these Unicode characters:
