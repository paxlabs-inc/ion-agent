...
version: 1.0.0
author: Ion Agent + Teknium (design systems sourced from VoltAgent/awesome-design-md)
license: MIT
...
54 production-ready design systems available for HTML/CSS generation. Each template captures the complete visual language of a site: color palette, typography hierarchy, component styles, spacing system, shadows, responsive behavior, and practical agent prompts with exact CSS values.
...
Each template includes a **Ion Implementation Notes** block at the top with:
...
<head>
  ...
  <!-- Paste the Google Fonts <link> from the template's Ion notes -->
...
Most sites use proprietary fonts not available via CDN. Each template maps to a Google Fonts
