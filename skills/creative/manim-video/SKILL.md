...
Apply this when users request animated explanations, math animations, concept visualizations, algorithm walkthroughs, technical explainers, 3Blue1Brown-style videos, or any programmatic animation involving geometric or mathematical content. Produces 3Blue1Brown-style explainer videos, algorithm visualizations, equation derivations, architecture diagrams, and data stories using Manim Community Edition.

## Creative Standard

This is educational cinema. Each frame teaches. Each animation reveals structure.

**Before writing any code**, articulate the narrative arc. What misconception does this address? What constitutes the "aha moment"? What visual journey takes the viewer from confusion to clarity? The user's prompt is a starting point — interpret it with pedagogical ambition.

**Geometry precedes algebra.** Display the shape first, then the equation. Visual memory encodes more rapidly than symbolic memory. When the viewer perceives the geometric pattern before the formula, the equation feels earned.

**First-render excellence is mandatory.** The output must be visually clear and aesthetically unified without revision rounds. If something appears cluttered, poorly timed, or resembles "AI-generated slides," it is wrong.

**Opacity layering directs attention.** Never display everything at full brightness. Primary elements at 1.0, contextual elements at 0.4, structural elements (axes, grids) at 0.15. The brain processes visual salience in layers.

**Breathing room.** Every animation requires `self.wait()` after it. The viewer needs time to absorb what appeared. Never rush from one animation to the next. A 2-second pause after a key reveal is never wasted.

**Cohesive visual language.** All scenes share a color palette, consistent typography sizing, and matching animation speeds. A technically correct video where each scene uses random different colors is an aesthetic failure.

## Prerequisites

Execute `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+, Manim Community Edition v0.20+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.20.1.
...
A single Python script per project. No browser, no Node.js, no GPU required.
...
1. **PLAN** — Create `plan.md` containing narrative arc, scene list, visual elements, color palette, and voiceover script
2. **CODE** — Write `script.py` with one class per scene, each renderable independently
...
6. **RENDER** — Render preview stills, verify against plan, adjust
...
**Monospace fonts are required for all text.** Manim's Pango renderer generates broken kerning with proportional fonts at every size. See `references/visual-design.md` for complete recommendations.
...
Never repeat identical config across all scenes. For each scene:
...
Before writing any code, create `plan.md`. See `references/scene-planning.md` for the comprehensive template.
...
When the user requests creative, experimental, or unconventional explanatory approaches, select a strategy and reason through it BEFORE designing the animation.
