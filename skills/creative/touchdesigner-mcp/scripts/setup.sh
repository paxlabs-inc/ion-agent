...
ION_HOME_DIR="${ION_HOME:-$HOME/.ion}"
ION_CFG="${ION_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Ion config has twozero_td MCP entry ──
if [[ ! -f "$ION_CFG" ]]; then
    echo -e " ${FAIL} Ion config not found at ${ION_CFG}"
    manual_steps+=("Create ${ION_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ION_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Ion config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Ion config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ION_CFG} manually"); }
    manual_steps+=("Restart Ion session to pick up config change")
fi
