...
`xurl` is the official CLI from the X developer platform for interacting with the X API. It provides shortcut commands for frequently-used actions as well as raw curl-style access to any v2 endpoint. All output is JSON to stdout.
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Ion tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Ion Docker layout, `/opt/data` is `ION_HOME`, but Ion tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Ion-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Ion tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Ion subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Ion adaptation: reformatted for Ion skill conventions; safety guardrails preserved verbatim.
