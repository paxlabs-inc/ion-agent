...
Keep track of blog and RSS/Atom feed updates using the `blogwatcher-cli` tool. It supports automatic feed discovery, HTML scraping fallback, OPML import, and read/unread article tracking.
...
By default the database resides at `~/.blogwatcher-cli/blogwatcher-cli.db`. In Docker this is lost when the container restarts. Use `BLOGWATCHER_DB` or a volume mount to persist it:
...
If upgrading from `Hyaxia/blogwatcher`, relocate your database:
...
Every flag can be configured via environment variables prefixed with `BLOGWATCHER_`:
...
- Automatically discovers RSS/Atom feeds from blog homepages when no `--feed-url` is specified.
- Falls back to HTML scraping when RSS fails and `--scrape-selector` is configured.
- Categories from RSS/Atom feeds are persisted and available for article filtering.
