...
Conduct code reviews on local changes before pushing, or review open PRs on GitHub. Most of this skill relies on plain `git` — the `gh`/`curl` split only matters for PR-level interactions.
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _ion_env="${ION_HOME:-$HOME/.ion}/.env"; [ -f "$_ion_env" ] && grep -q "^GITHUB_TOKEN=" "$_ion_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ion_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Ion Agent\",
    \"comments\": [
...
```bash
source "${ION_HOME:-$HOME/.ion}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Ion Agent. Code looks clean — good test coverage, no security concerns."
