...
func buildBrief(wf *Doc) string {
	...
	for _, kv := range wf.OrderedKV("memory", "") {
		fmt.Fprintf(&b, "- **%s**: %s\n", humanize(kv[0]), kv[1])
	}
...
func adapter(label, brief, banner string) string {
	...
	case "cursor":
		...
		return fm + cmt + brief
	case "devin_workflow":
		// Devin workflow: a slash-command (/spec) with a description.
		fm := "---\n" +
			"description: Run the Ion spec workflow (recall, then drive the active spec.kvx task list)\n" +
			"---\n\n"
		return fm + cmt + brief
	case "kiro":
		...
		return fm + cmt + brief
	default:
		// devin_rules, claude (CLAUDE.md), codex (AGENTS.md), copilot — plain md.
		return cmt + brief
	}
}
